...
-- issued_status == members == books == return_sataus
-- filter books which is return 
-- overdue of 30

CREATE TABLE OVER_DUE_BOOKS(
SELECT m.member_id,m.member_name,b.book_title, ist.issued_date AS issued_dt,
...
values ('RS125','IS140',CURRENT_DATE,'Good');
...
DELIMITER $$
CREATE PROCEDURE add_return_records(IN p_retunr_id VARCHAR(5),IN p_issued_id VARCHAR(5),IN p_book_quality VARCHAR(15))
...
-- ALL YOUR LOGIC AND CODE 
	INSERT INTO return_status (return_id,issued_id,return_date,book_quality)
	values (p_retunr_id,p_issued_id,CURRENT_DATE,p_book_quality);
    ...
    SELECT 'THANK YOU FOR SUBMITING THE BOOK : ',v_book_name;
...
DROP TABLE Branch_Wise_Revenue;
CREATE TABLE Branch_Wise_Revenue(
SELECT b.branch_id,COUNT(distinct ist.issued_id) AS Issued_books_count, COUNT(distinct rs.return_id) AS Returned_books_count,
...
SELECT e.emp_id,b.*,COUNT(ist.issued_id) AS total_books_issued FROM issued_status AS ist
JOIN employees AS e
ON ist.issued_emp_id = e.emp_id
JOIN branch AS b
ON b.branch_id = e.branch_id
...
SELECT 'Book record added successfully for book isbn: ' ,p_issued_book_isbn;
ELSE

SELECT 'SORRY, the book which you have requested is not available right now!';
...
DELIMITER ;
CALL issue_book('IS182','C107','978-0-06-025492-6','E106');
DELETE FROM issued_status WHERE issued_id = '180' AND '181';
UPDATE books
SET status='no'
WHERE isbn='978-0-06-025492-6';

CALL issue_book('IS180','C107','978-0-375-41398-8','E106');
